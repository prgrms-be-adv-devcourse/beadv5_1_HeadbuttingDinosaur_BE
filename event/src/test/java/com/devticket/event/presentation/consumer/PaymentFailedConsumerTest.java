...
class PaymentFailedConsumerTest {
    ...
    private ConsumerRecord<String, String> createRecord(UUID messageId, String payload) {
        ...
        return new ConsumerRecord<>("payment.failed", 0, 0L, 0L,
    ...
    class NormalProcessing {
        ...
        void success_thenAck() {
            ...
            doNothing().when(stockRestoreService)
                .restoreStockForPaymentFailed(any(), any(), any());

            consumer.consume(record, ack);

            verify(stockRestoreService).restoreStockForPaymentFailed(
                eq(messageId), eq("payment.failed"), eq("{}"));
    ...
    class MissingHeader {
        ...
        void noHeader_throwsException() {
            ...
            ConsumerRecord<String, String> record = new ConsumerRecord<>(
                "payment.failed", 0, 0L, 0L, TimestampType.CREATE_TIME,
    ...
    class OptimisticLockHandling {
        ...
        void optimisticLock_alreadyProcessed_thenSkip() {
            ...
            doThrow(new ObjectOptimisticLockingFailureException("Event", messageId))
                .when(stockRestoreService).restoreStockForPaymentFailed(any(), any(), any());
