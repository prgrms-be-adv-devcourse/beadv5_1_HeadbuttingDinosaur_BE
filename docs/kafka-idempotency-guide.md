...
ProducerRecord<String, String> record = new ProducerRecord<>(
        outbox.getTopic(),
        outbox.getAggregateId(),   // partition key
        outbox.getPayload()
    );
...
public class OrderService {
    ...
    public void processStockDeducted(UUID messageId, String topic, String payload) {
        ...
        StockDeductedEvent event = objectMapper.readValue(payload, StockDeductedEvent.class);
        ...
        if (!order.canTransitionTo(OrderStatus.STOCK_RESERVED)) {
            if (isExplainableSkip(order.getStatus())) {
                // 정책적 스킵: 만료·보상 등으로 CANCELLED 등 도달
                deduplicationService.markProcessed(messageId, topic);
                return;
            }
            // 이상 상태: 설명 불가능 → throw → 재시도 → DLT
            throw new IllegalStateException(
                "Invalid transition: " + order.getStatus() + " -> STOCK_RESERVED"
            );
        }
...
|----------|-----------|----------------|----------|------------|-----------|
| Commerce | stock.deducted | CREATED | 이미 STOCK_RESERVED | CANCELLED, FAILED | PAID, REFUNDED |
| Commerce | stock.failed | CREATED | 이미 FAILED | CANCELLED | PAID, REFUNDED |
| Commerce | payment.completed | STOCK_RESERVED / PAYMENT_PENDING / PG_PENDING | 이미 PAID | — | CREATED, FAILED |
| Commerce | payment.failed | PAYMENT_PENDING / PG_PENDING | 이미 FAILED | CANCELLED | CREATED, PAID |
| Event | order.created | 재고 > 0 | 이미 차감됨 (중복 consume) | SOLD_OUT → stock.failed 발행 | — |
| Event | payment.failed | 재고 선점 상태 | 이미 복구됨 | 선점 안 했으면 스킵 | — |
| Payment | ticket.issue-failed | 결제 SUCCESS | 이미 환불됨 | 결제 FAILED면 스킵 | — |
...
public enum StockStatus {
    ...
    public boolean canTransitionTo(StockStatus target) {
        return switch (this) {
            case RESERVED   -> target == DEDUCTED || target == RELEASED;
            case DEDUCTED   -> target == RELEASED;
            case RELEASED   -> false;  // 종단 상태
        };
    }
}
```
...
|------------|----------|----------|
| payment.failed → 재고 복구 | Event | stock 상태가 RELEASED이면 스킵 (`canTransitionTo()`로 커버, 별도 플래그 불필요) |
| ticket.issue-failed → 자동 환불 | Payment | Payment 상태가 REFUNDED이면 스킵 (`canTransitionTo()`로 커버) |
| ticket.issue-failed → 재고 복구 | Event | stock 상태가 RELEASED이면 스킵 (`canTransitionTo()`로 커버) |
| event.force-cancelled → 결제 취소 | Payment | Payment 상태가 REFUNDED이면 스킵 (`canTransitionTo()`로 커버) |
...
```
주문 만료 스케줄러: CREATED → CANCELLED + 재고 복구 Outbox 저장
동시에 Commerce Consumer: stock.deducted 수신 → CREATED → STOCK_RESERVED
...
2. **상태 전이 유효성**: Consumer가 상태 확인 → 이미 CANCELLED면 stock.deducted는 정책적 스킵
3. **스케줄러도 상태 확인**: 이미 STOCK_RESERVED면 만료 처리 안 함
...
> groupId 네이밍 패턴 및 전체 매핑 표(14개)는 [kafka-design.md §5 — Consumer 멱등성 설계](kafka-design.md#5-consumer-멱등성-설계) 참조
