...
>
> ⚠ 자동 자산 드리프트 정정 이력: 이전 자동 파서가 dead REST 2건을 잔존 등재했으나, ✅ `api-overview.md` 부록 #1 (b9be8434 근거) 로 제거 통합됨. 이전 `api-summary.md/json` 자동 자산은 폐기. 회귀 가이드는 `docs/standards/docs-parser-standard.md §신규 변경 미반영` 참조.
