...
|---|---|---|---|---|---|
| Recommendation Internal | POST | `/internal/ai/recommendation` ★ | `RecommendationController#recommend` | event 모듈 (`/api/events/user/recommendations` 위임 추정) | (#10, §2 AI 추천 + 벡터DB) 사용자 추천 이벤트 ID 목록 반환. 내부적으로 `recommendByUserVector` 호출, `UserVector` 없으면 `recommendByColdStart` 로 폴백 |
...
- **REST 호출**:
  ...
  - 외부: Elasticsearch (KNN 검색 — `ElasticsearchClient`) ★ (§2 벡터DB), `TechStackEmbeddingRepository` (벡터 저장소 — admin 모듈 흐름이 임베딩 산출)
- **Kafka 구독**: 없음
...
- **Elasticsearch KNN 인덱스** ★ (§2 벡터DB): `ElasticsearchIndexInitializer` 가 부팅 시 인덱스 초기화 (1536차원 dense_vector, similarity=cosine, k=30, status=ON_SALE 필터)

### ⚠ 미결

- **K8s HPA 미설정** ★ (§2 K8s — ServiceOverview §5 ⚠1): HPA 매니페스트 부재, `kubectl scale --replicas=1` 수동 운영
