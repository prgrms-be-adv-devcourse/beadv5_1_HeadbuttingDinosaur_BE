...
- **JWT 토큰 페이로드** ★ (§2 JWT): 인증 정보(`X-User-Id` 등)는 JWT claims 로 다운스트림에 헤더 형태로 전달 — DTO 가 아님
- **OAuth 토큰 교환 페이로드** ★ (§2 OAuth): Google OAuth 콜백에서 받는 인증코드 → 토큰 교환 — `member` 모듈로 위임 (`internalOAuthSignUpOrLogin`), DTO 정의는 member 측
