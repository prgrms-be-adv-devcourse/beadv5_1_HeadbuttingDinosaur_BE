...
### 2-4. Outbox 스케줄러 (fallback 경로)

- **2-경로 발행 구조**: 정상 흐름은 `OutboxAfterCommitPublisher` (트랜잭션 afterCommit 즉시 발행) 가 처리. Scheduler 는 누락/재시도 fallback.
- **폴링 주기: 60초** (Commerce `devticket.outbox.scheduler-delay-ms=60000`, Event/Payment `outbox.poll-interval-ms:60000`)
- 조회 조건: PENDING + `next_retry_at < now` + `created_at < graceCutoff` (기본 5초 grace — afterCommit 진행 중 레코드와 경합 회피)
- Kafka 발행 성공 → SENT 상태 전환 (`PROPAGATION_REQUIRES_NEW`)
- Kafka 발행 실패 → `retry_count++`, `next_retry_at` 갱신 (지수 백오프)
...
- **UNIQUE 제약: `(message_id)` 단독** — Outbox `UUID.randomUUID()`를 Kafka 헤더(`X-Message-Id`)로 전달받아 저장, 전역 고유이므로 topic 복합키 불필요
 ...
 - Consumer UNIQUE 충돌 catch 시 이 제약명으로 비교 (예: 활성 Consumer 의 `isProcessedMessageUniqueConflict()` 헬퍼 — `PaymentFailedConsumer` / `RefundOrderConsumer` 등)
...
Consumer의 dedup 키는 **Outbox가 생성한 `UUID.randomUUID().toString()`** 입니다. **3모듈 통일: `String` / `VARCHAR(36)`** (PR #584 — 2026-04-27, Payment 측 컬럼·필드를 UUID → String 정렬 + 운영 ALTER 함께 실행).
...
|---|---|---|---|
| Payment | `wallet/application/event/` (`PaymentCompletedEvent`, `RefundCompletedEvent`, `PaymentFailedEvent`. `EventCancelledEvent`는 잔존하나 사용처 없음) | `wallet/infrastructure/kafka/WalletEventConsumer.java` | `common/messaging/event/` 미사용 — 도메인(wallet) 모듈 하위 배치 |
| Event | `common/messaging/event/` (`PaymentFailedEvent`, `EventForceCancelledEvent`, `EventSaleStoppedEvent`, `RefundCompletedEvent`, `ActionLogEvent` 등) | `presentation/consumer/PaymentFailedConsumer.java`, `OrderCancelledConsumer.java` 등 | `KafkaTopics`도 `common/messaging/` 아래 (F2 결정 — 3모듈 공통화) |
...
| Consumer | **Log 서비스** (Fastify/TS, `fastify-log/`) + **AI 모듈** (`ai/infrastructure/kafka/ActionLogConsumer.java`, groupId `ai.action-log` — 행동 시퀀스 기반 추천 학습) |
