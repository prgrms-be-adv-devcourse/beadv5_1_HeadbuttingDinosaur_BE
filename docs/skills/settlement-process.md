...
| (계산 입력) | `total_fee_amount` : `Integer` | `total_sales_amount × FeePolicy.feeValue / 100`. `FeePolicy` 엔티티(`fee_policy` 테이블, `feeType=PERCENTAGE`, `feeValue: BigDecimal`)에서 정책 조회. 일별 SettlementItem 생성 시점(`DailySettlementTasklet`)에 적용 |
