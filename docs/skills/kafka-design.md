...
|---|---|---|
| Commerce | `ticket.issue-failed`, `refund.requested` (fan-out), `refund.order.done`, `refund.order.failed`, `refund.ticket.done`, `refund.ticket.failed`, `order.cancelled`, `action.log` (CART_ADD / CART_REMOVE) | `payment.completed`, `payment.failed`, `refund.completed`, `event.force-cancelled`, `refund.order.cancel`, `refund.ticket.cancel`, `refund.order.compensate`, `refund.ticket.compensate` |
| Event | `event.force-cancelled`, `event.sale-stopped`, `refund.stock.done`, `refund.stock.failed`, `action.log` (VIEW / DETAIL_VIEW / DWELL_TIME) | `order.cancelled`, `payment.failed`, `refund.completed`, `refund.stock.restore` |
| Payment (Orchestrator 포함) | `payment.completed`, `payment.failed`, `refund.completed`, `refund.order.cancel`, `refund.ticket.cancel`, `refund.stock.restore`, `refund.order.compensate`, `refund.ticket.compensate` | `refund.completed` (dedup 마킹 전용 — 잔액 복구는 Orchestrator 직접 호출), `ticket.issue-failed`, `refund.requested`, `refund.order.done`, `refund.order.failed`, `refund.ticket.done`, `refund.ticket.failed`, `refund.stock.done`, `refund.stock.failed` |
...
| **AI** (추천 모듈) | — | `action.log` (groupId `ai.action-log`, 행동 시퀀스 기반 추천 학습) |

> ⚠️ **Payment Consumer 책임 범위**: Payment는 `event.force-cancelled` / `event.sale-stopped` 토픽을 **직접 소비하지 않는다**. 이벤트 강제취소 fan-out은 Commerce(`EventForceCancelledConsumer` + `RefundFanoutService`)가 담당 → orderId별 `refund.requested`로 분배 → Payment Orchestrator 가 `refund.requested`만 수신. Payment의 `WalletEventConsumer`는 `refund.completed` 1개 토픽만 소비하며 dedup 마킹 전용(잔액 복구는 `RefundSagaOrchestrator`가 `WalletService.restoreBalance()` 직접 호출).
...
### EventForceCancelledEvent / EventSaleStoppedEvent

> 두 이벤트는 **별도 record로 분리**되어 있다 (Action A/B 의도 분리).
> 단일 공용 record(예전 `EventCancelledEvent`)는 폐기.

```java
// event/common/messaging/event/EventForceCancelledEvent.java
// 토픽: event.force-cancelled — Action A 강제취소 (환불 동반)
public record EventForceCancelledEvent(
  UUID eventId,
  UUID sellerId,
  String reason,     // 취소 사유 (admin / seller 입력값)
  Instant occurredAt
) {}

// event/common/messaging/event/EventSaleStoppedEvent.java
// 토픽: event.sale-stopped — Action B 판매 중지 (환불 없음)
...
> ⚠️ **호출자/role 식별은 payload가 아니라 발행 시점 컨텍스트로 처리**: `cancelledBy` enum / `adminId` 필드는 코드에 도입하지 않음. Action A는 admin·seller 양쪽 모두 동일 토픽 `event.force-cancelled`로 발행되며, 환불 fan-out은 Commerce가 단일 핸들러로 처리.
> ⚠️ **Commerce 측 record 사본**: `commerce/common/messaging/event/refund/EventForceCancelledEvent.java` (Consumer DTO 복사본, 동일 JSON 스키마).
> ⚠️ Payment 모듈에 `wallet/application/event/EventCancelledEvent.java`가 잔존하지만 사용처 없음 — 후속 정리 대상.
...
>
> ⚠️ **모듈 간 record 타입 차이 (현 시점 실제 코드)**:
> - **Commerce** `commerce/common/messaging/ActionLogEvent.java`: `UUID userId, UUID eventId, ActionType actionType` (계약 표준)
> - **Event** `event/common/messaging/event/ActionLogEvent.java`: `String userId, String eventId, String actionType` (String 기반)
> - JSON 직렬화 시 둘 다 동일 형태이며 Fastify Consumer는 String 기반이라 우연히 호환. 다만 Java 측 모듈 간 타입 불일치는 **후속 정리 대상** (Event 측을 UUID/ActionType으로 정렬). 본 절의 record 정의는 Commerce 기준 = 권장 표준.
...
public enum ActionType {
  ...
  CART_ADD,   // 장바구니 담기
  CART_REMOVE, // 장바구니 삭제
  PURCHASE,   // 결제 (payment.completed 수신 시 Log 서비스가 직접 INSERT)
...
```java
// Commerce: @Scheduled(fixedDelayString = "${devticket.outbox.scheduler-delay-ms}") // application.yml: 60000 (60초)
// Event/Payment: @Scheduled(fixedDelayString = "${outbox.poll-interval-ms:60000}")  // 기본 60초
@SchedulerLock(name = "outbox-scheduler", lockAtMostFor = "5m", lockAtLeastFor = "5s")
public void publishPendingEvents() { ... }
```

> ⚠️ **2-경로 발행 구조** — Outbox는 단일 Scheduler 폴링이 아니라 **AfterCommit 즉시 발행 + Scheduler fallback** 의 dual-path로 동작한다:
> - **주 경로**: `OutboxAfterCommitPublisher` (3모듈 모두 보유) — `TransactionSynchronization.afterCommit()` 훅에서 별도 executor로 즉시 Kafka 발행 → 정상 흐름은 60초 폴링을 기다리지 않는다.
> - **Fallback 경로**: `OutboxScheduler` 60초 주기 — afterCommit 발행 실패/누락 또는 재시도 대상 픽업. Repository 쿼리에 `createdAt < :graceCutoff` 조건(기본 5초 grace)을 둬 afterCommit 진행 중 레코드와의 경합을 회피.
> - `markSent`는 `PROPAGATION_REQUIRES_NEW` 정책으로 별도 트랜잭션 처리.
>
> 이전 문서 표기였던 `fixedDelay=3000` (3초)은 dual-path 도입 이전 설정이며, 현재 코드/yml 기준 **60초**가 정답.
>
...
스케줄러 쿼리: `next_retry_at < now()` (경계 배제) + `created_at < :graceCutoff` (afterCommit 발행 중 레코드 제외).

```java
// OutboxRepository (실제 시그니처 — 3-인자)
...
> `graceCutoff = now - publish-grace-seconds` (기본 5초). `OutboxAfterCommitPublisher`가 발행 중인 신규 레코드를 Scheduler가 동시에 픽업하지 않도록 하는 안전 장치.
...
> `approve()` / `fail()` / `cancel()` / `refund()` 메서드 진입 시 `validateTransition()` 가드 호출.
...
> **현재 상태:**
> - **Order**: `order/domain/model/Order.java:200-214` (CREATED→PAYMENT_PENDING/FAILED, PAYMENT_PENDING→PAID/FAILED/CANCELLED, PAID→CANCELLED) + 각 상태 전이 메서드 내부 가드 호출
> - **Payment**: `PaymentStatus.canTransitionTo()` + `approve()` / `fail()` / `cancel()` / `refund()` 내부 가드 호출
...
```
topic: payment.completed   → DLT: payment.completed.DLT
...
| 2 | dedupe 키 스코프 (message_id+topic 복합키는 fan-out 구조에서 Consumer 간 dedup 공유 문제 발생 → 모듈별 별도 테이블로 구독자 스코프 격리) | 격리 단위 = 서비스 DB, Outbox UUID를 Kafka 헤더로 전달 | `X-Message-Id` 헤더 + 모듈별 `processed_message` + `UNIQUE(message_id)` | 3모듈(Commerce/Event/Payment) |
| 3 | 메시지 순서 역전 | 3분류 처리 — ① 이미 목표 상태: 멱등 스킵 + ACK ② 설명 가능한 순서 역전(만료·보상 등): 정책적 스킵 + ACK ③ 설명 불가능한 상태: throw → 재시도 → DLT (단순 스킵 금지 — 정합성 문제 소거 위험) | `canTransitionTo()` + 예외 타입 3분류 | Commerce 핵심 Consumer(`OrderService.processPaymentCompleted/Failed`) / Payment·Event 일부 적용 |
| 4 | Producer 재발행 시 dedupe | `Outbox.messageId` 생성 시 고정, 재발행 시 동일 ID | 현재 구현 유지 | 구현됨, 문서화 누락 |
| 5 | Outbox 락·재시도 | ShedLock 채택, `next_retry_at` 컬럼 추가 | ShedLock(`lockAtMostFor=5m`, `lockAtLeastFor=5s`) + 지수 백오프 6회(`2^(retryCount-1)초` = 즉시/1/2/4/8/16초, 누적 31초) → `retryCount >= 6` 도달 시 FAILED | ShedLock — 3개 모듈 정책 통일 (2026-04-22) |
...
| 8 | 상태 전이 검증 없음 | processed_message + 상태 전이 검증 두 방어선 필수 | Case 3 동일, 예외 타입 분리 | Commerce 핵심 Consumer / 기타 Consumer 점진 적용 |
...
- `Outbox`: 엔티티 필드 — `aggregate_id` VARCHAR(36), `topic`, `partitionKey`, `nextRetryAt`, `sentAt`
- `OutboxRepository`: `next_retry_at IS NULL OR next_retry_at < :now` 조건 (Commerce 기준으로 연산자 통일 — 2026-04-22)
- `OutboxScheduler`: ShedLock 적용 + `getTopic()` / `getPartitionKey()` 사용
- `ProcessedMessage`: `topic VARCHAR(128)` 컬럼
- `PaymentCompletedEvent`: record, `UUID` / `PaymentMethod enum` / `Instant`
- `RefundCompletedEvent`: record, `UUID` / `PaymentMethod enum` / `Instant`
- ~~`EventCancelledEvent`~~ 폐기 — `wallet/application/event/`에 record 잔존하나 사용처 없음(후속 제거 대상). Event 모듈이 `EventForceCancelledEvent` + `EventSaleStoppedEvent` 두 record 분리 사용 (§3 참조)
- `PaymentFailedEvent`: record
- `PaymentStatus.canTransitionTo()` 상태 전이 검증
- Payment 엔티티 낙관적 락 (`@Version`)
- `WalletServiceImpl.processWalletPayment()`: `payment.completed` Outbox 이벤트 발행으로 전환
- `OutboxEventProducer`: Kafka 발행 시 `X-Message-Id` 헤더 세팅
...
- `WalletEventConsumer`: `markProcessed()`를 `RefundCompletedHandler`의 단일 `@Transactional`로 이동
- Payment 엔티티 `approve()` / `fail()` / `cancel()` / `refund()` 내부에 `validateTransition()` 가드 호출
...
**구현 (주문생성 ~ 결제완료 Phase)**
- `JacksonConfig` (`common/config/JacksonConfig.java`) — 패키지는 Commerce 포함 3모듈 모두 `common/config/`. `infrastructure/config/*` 이전은 미수행 (AGENTS.md §2.1 정합 위해 추후 리팩터링 트랙 예정)
- `KafkaTopics` 상수 클래스 — `common/messaging/KafkaTopics.java` (Saga 6개 + 환불 1개 + 이벤트 관리 2개 + Orchestration 12개 + `ORDER_CANCELLED` + `ACTION_LOG` = 22개)
- Outbox 패턴 구현 (`Outbox`, `OutboxService`, `OutboxEventProducer`, `OutboxScheduler` + ShedLock)
- `MessageDeduplicationService` + `processed_message` 테이블 (제약명: `uk_processed_message_message_id_topic`, 실제 `message_id` 단독 UNIQUE)
- ShedLock 적용 (`OutboxScheduler` / `OrderExpirationScheduler`)
- Consumer dedup 패턴 적용 — `PaymentCompletedConsumer`, `PaymentFailedConsumer`, `OrderRefundConsumer`, `TicketRefundConsumer`, `EventForceCancelledConsumer` (※ `StockEventConsumer`는 REST 동기 차감 전환으로 폐기)
- 상태 전이 검증 `canTransitionTo()` — `Order.java:200-214` (CREATED→PAYMENT_PENDING/FAILED, PAYMENT_PENDING→PAID/FAILED/CANCELLED, PAID→CANCELLED)
- Order 엔티티 낙관적 락 (`@Version`)
- 주문 만료 스케줄러 + 재고 복구 Outbox 발행 (`OrderExpirationScheduler` + `OrderExpirationCancelService`)
- 결제 완료 시 장바구니 매칭 차감 (A안, #427/#436)
...
**구현 완료**
- `KafkaTopics` 상수 클래스 (`common/messaging/KafkaTopics.java`) — Consumer/Producer 토픽 모두 선반영
 > ⚠️ `infrastructure/messaging/KafkaTopics.java` 사본이 로컬 untracked로 존재 — 사용처 없음, 후속 정리 필요
- `JacksonConfig` 빈 (`common/config/JacksonConfig.java`) — Jackson 2 `ObjectMapper` `@Primary`, `JavaTimeModule` + `WRITE_DATES_AS_TIMESTAMPS` disable
 > 참고: `StockRestoreService`는 Jackson 3(`tools.jackson`) `JsonMapper`도 병행 DI. Spring Boot 4 auto-config이 자동 등록 — Jackson 2/3 공존 상태
- `KafkaProducerConfig` — `acks=all`, `enable.idempotence=true`, `retries=3`, `max.in.flight.requests=5`, StringSerializer
- `ShedLockConfig` — `JdbcTemplateLockProvider` + `.usingDbTime()`, `lockAtMostFor=30s`
- `KafkaConsumerConfig` — `ExponentialBackOff(2→4→8초, 3회)` + `AckMode.MANUAL`
- `MessageDeduplicationService` 구현 + `processed_message` 테이블 (`topic` 컬럼 포함, schema=`event`)
- Outbox 패키지 전체 (`common/outbox/`) — `Outbox` 엔티티, `OutboxStatus`, `OutboxEventMessage`, `OutboxRepository`(nextRetryAt 조건 + graceCutoff 포함, 3-인자), `OutboxService`(save MANDATORY 트랜잭션), `OutboxScheduler`(`@SchedulerLock`, `fixedDelayString="${outbox.poll-interval-ms:60000}"`=60초, 지수 백오프 6회: 1/2/4/8/16s, 7번째 FAILED), `OutboxAfterCommitPublisher`(afterCommit 즉시 발행, dual-path 주 경로), `OutboxEventProducer`(X-Message-Id 헤더 + partition key)
- `Event` 엔티티 낙관적 락 (`@Version`) 적용
- 이벤트 DTO record 신규 (`common/messaging/event/`): `EventForceCancelledEvent`, `EventSaleStoppedEvent`, `PaymentFailedEvent(List<OrderItem>)`, `RefundCompletedEvent`, `ActionLogEvent` 등 — UUID/Instant/enum 타입 준수 (단 ActionLogEvent는 String 기반 — 본 §3 ActionLogEvent 호환성 노트 참조)
- `payment.failed` Consumer (`PaymentFailedConsumer` + `StockRestoreService`) — dedup, EventStatus 정책적 스킵, 비관적 락 정렬 조회, `markProcessed()` 트랜잭션 내부, `@Version` / UNIQUE 충돌 핸들링, groupId `event-payment.failed`
- `order.cancelled` Consumer (`OrderCancelledConsumer`, groupId `event-order.cancelled`) — Order 만료/실패 시 재고 복구 처리

> ⚠️ `order.created` Consumer / `EventService.processOrderCreated()` / `saveStockFailed()` / `StockDeductionException` / `stock.deducted` · `stock.failed` Outbox 발행은 **REST 동기 차감 전환**(`OrderToEventClient.adjustStocks` → `PATCH /internal/events/stock-adjustments`)으로 폐기. 코드에 해당 클래스/메서드 없음.
