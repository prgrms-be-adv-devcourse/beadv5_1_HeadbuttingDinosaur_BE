...
> 📝 **본 문서는 2026-04-22 작업 이력 기록입니다.** Outbox 정합 A 파트는 PR #482/#483/#484 머지로, 후속 B 트랙도 정리됨. 현재 운영 중인 Outbox 패턴 설계는 [`kafka-design.md` §4](kafka-design.md) · 모듈별 적용 현황은 [`../ServiceOverview.md` §5-1](../ServiceOverview.md) 참조.
>
...
- PR #482 (Commerce) / #483 (Payment) / #484 (Event) **A 파트** — 이중 발행 완화 3축(ShedLock / Producer 타임아웃 / Scheduler 트랜잭션) 정합
...
| 발행 주 경로 | `OutboxAfterCommitPublisher` (3모듈 모두 보유) — `TransactionSynchronization.afterCommit()` 훅에서 별도 executor로 즉시 Kafka 발행 |
| Scheduler `@Scheduled` 주기 | **60초** (Commerce `devticket.outbox.scheduler-delay-ms=60000` / Event·Payment `outbox.poll-interval-ms:60000`) — fallback 경로 |
| `@SchedulerLock(lockAtMostFor)` | `"5m"` |
| `@SchedulerLock(lockAtLeastFor)` | `"5s"` |
| Scheduler 트랜잭션 경계 | **루프만** — 건별 트랜잭션은 `processOne()` 별도 빈에서. 장기 `@Transactional` 금지, self-invocation 금지. `markSent`는 `PROPAGATION_REQUIRES_NEW` |
| Repository 메서드명 | `findPendingToPublish` |
| Repository 쿼리 시그니처 | **3-인자**: `(status, now, graceCutoff)` — `nextRetryAt < :now` + `createdAt < :graceCutoff` (afterCommit 경로와 경합 회피, 기본 grace 5초) |
| Repository LIMIT | `50` |
| 재시도 최대 횟수 | **6회 지수** (`2^(retryCount-1)초` = 즉시/1/2/4/8/16s, 누적 31초) |
| 실패 메서드명 | `markFailed()` (Payment·Event는 상수 내부화 / **Commerce는 리터럴 `6` 사용** — 후속 통일 필요) |
...
- `@Table(schema="commerce")` — 적용
- `messageId` String(36) — 적용 (`length=36`, UUID.toString())
- `nextRetryAt` 필드 — 있음
- `save(aggregateId, partitionKey, eventType, topic, event) + @Transactional(MANDATORY)` — 적용
- `processOne()` 별도 빈 — 적용
- `publish(OutboxEventMessage): void throws OutboxPublishException` — 적용
- `findPendingToPublish` + `< :now` — 적용
- ⚠ `markFailed()` 상수 내부화 — Commerce는 `markFailed()` 본문에 리터럴 `6` 사용. Payment `MAX_RETRY=6` / Event `MAX_RETRIES=6`만 상수화. Commerce 측 상수화는 후속 정리 대상.
- Scheduler `@Transactional` 없음, `lockAtMostFor=5m / lockAtLeastFor=5s` — 적용
- ProducerConfig 3종 타임아웃 (`1500/1000/500`) — PR #482에서 적용
- 회귀 방지 테스트 — `KafkaProducerConfigTest` + `OutboxSchedulerIntegrationTest` (PR #482, `e1d53c8`)
- 패키지 경로 `commerce.common.outbox` 정착
...
> 선례 소스: `origin/develop/commerce` (PR #482 `e1d53c8`)
...
#### 선례 (작업 없음)
- **B1 선례**: `nextRetryAt` 필드 + 재시도 6회 지수 백오프 (Scheduler 주석 명시)
- **B2 선례**: `save(aggregateId, partitionKey, eventType, topic, event) + @Transactional(MANDATORY)`
- **B4 선례**: `@Table(schema="event")` + `messageId` String(36)
- `processOne()` 별도 빈 / Scheduler `@Transactional` 없음 / `lockAtMostFor=5m` — PR #484 A파트
...
- **B5-1 Repository 메서드명 통일**: `findPendingOutboxes` → `findPendingToPublish`
- **B5-2 쿼리 연산자 통일**: `<= :now` → `< :now` (경계 배제)
- **B5-3 `markFailed()` 상수 내부화**: `markFailed(MAX_RETRIES)` → `markFailed()`, 상수 `Outbox` 엔티티 내부 이전
- **B6 회귀 방지 테스트 이식** (Commerce 선례 복제)
...
- `event/src/main/java/com/devticket/event/infrastructure/messaging/` 죽은 복사본 제거
 - `KafkaTopics.java` (구버전, `ACTION_LOG` 누락) / `event/PaymentFailedEvent.java` (내용 동일)
 - `common/messaging/` 최신 본체 참조 0건 외부 → 삭제 안전

---

### 3-C. Payment — A파트 + B1~B7 + T2/T3 **완료**

> PR #483 A 파트(ShedLock 5m / Producer 타임아웃 3종 / `.get(2s)` / Scheduler 트랜잭션 해소 / `processOne()` 분리).
> **B1~B5 + B6 후속 수거 완료** (commits `4fc0a20` refactor / `c70ba7e` test).
> B7 (Commerce 선례 `@Value` 외부화) + T2/T3 (고가치 회귀 방지 테스트) 함께 반영.

#### 선례 (A 파트 이후)
- `nextRetryAt` 필드 — 있음
- `processOne()` 별도 빈 — A 파트 적용
- Scheduler `@Transactional` 없음 / `lockAtMostFor=5m / lockAtLeastFor=5s` — A 파트 적용
- ProducerConfig 3종 타임아웃 (`1500/1000/500`) + `retries=3` 제거 — A 파트 적용 (B7에서 `@Value` 외부화로 승격)
- `.get(2s)` — A 파트 적용 (B7에서 `sendTimeoutMs` `@Value` 외부화)

#### 후속 (완료)

- **B1 재시도 6회 지수 백오프** — `4fc0a20`
...
- **B2 `OutboxService.save()` 시그니처 재정렬 + MANDATORY 전파** — `4fc0a20`
...
- **B3 Producer `publish()` 시그니처 전환** — `4fc0a20`
...
- **B4-1 `@Table(schema="payment")` 적용** — `4fc0a20`
- **B4-2 `messageId` 타입 전환** — `4fc0a20`
 - 엔티티/호출부/Kafka 헤더 전부 `String` 일관
 - **⚠️ 별건 이슈**: 운영 DB 컬럼 UUID→VARCHAR(36) 마이그레이션 스크립트 분리 필요 (PostgreSQL `USING message_id::text` 명시)
- **B5-1 Repository 메서드명 통일** — `findPendingToPublish` — `4fc0a20`
- **B5-2 쿼리 연산자 통일** — `< :now` (경계 배제) — `4fc0a20`
- **B5-3 `markFailed()` 상수 내부화** — `MAX_RETRY=6` 엔티티 내부화 — `4fc0a20`
- **B6 회귀 방지 테스트 이식 (Commerce 선례)** — `c70ba7e`
...
- **B7 Commerce 선례 — Producer 타임아웃 `@Value` 외부화** — `4fc0a20`
...
- **T2 `@Transactional(MANDATORY)` 불변식 E2E** — `c70ba7e`
...
- **T3 Repository 쿼리 경계 + LIMIT** — `c70ba7e`
...
- 엔티티 필드 / Kafka 헤더 / 호출부 전부 `String` 일관 — 코드 검증
- [ ] **운영 DB 컬럼 UUID→VARCHAR(36) 마이그레이션 — outbox 측** — `schema_plan.md` ⑦번 SQL, 운영 DB 적용 별도 확인 (PostgreSQL `USING message_id::text` 명시 필요)
- **`processed_message.message_id` 타입 정합** PR #584 머지 (2026-04-27): Consumer 측 엔티티 필드 `UUID → String` + 운영 DB ⑧번 ALTER 함께 실행. 3모듈 통일

### 5-D. 재시도 정책 전환 영향 — Payment **완료** (B1)

- `MAX_RETRY=6` 상수 엔티티 내부화 + `markFailed()` 구현 교체
...
- Commerce `KafkaProducerConfigTest` / `OutboxSchedulerIntegrationTest` — PR #482
- Event 이식 — PR #484 후속
- Payment 이식 — `c70ba7e` (`KafkaProducerConfigTest` + `OutboxSchedulerIntegrationTest` + `OutboxServicePropagationTest` + `OutboxRepositoryTest`)
...
- `OutboxRepositoryTest` — `< :now` 경계 4케이스 (NULL / past / equal-skip / future-skip) + SENT 배제 + LIMIT 50 배치 상한
...
- 관련 PR: **#482 Commerce** / **#483 Payment** / **#484 Event** (A 파트)
...
- **Payment 후속 완료** (2026-04-22) — B1~B7 전 항목 + T2/T3 고가치 회귀 방지 테스트 — commits `4fc0a20` (refactor) / `c70ba7e` (test). A 파트는 PR #483
