...
| processed_message 수정 | `message_id` | UUID → VARCHAR(36) 타입 변경 **PR #584** (2026-04-27, 엔티티 필드도 `String` 으로 통일) | 수동 ALTER (PostgreSQL `USING message_id::text` 명시 필요) |
...
| order 엔티티 | `cart_hash` | VARCHAR(64) 컬럼 추가, 인덱스 (user_id, cart_hash), 해시 대상: (eventId, quantity) — unitPrice 미포함 (팀 합의) (2026-04-19) | 엔티티 필드 추가 → 자동 |
| cart_item 엔티티 | `(cart_id, event_id)` UNIQUE | 광클 동시성 결함 방어 + cart_hash 분기 삭제(A안) 매칭 로직 단순화 | 엔티티 필드 추가 → 자동 |

> **코드 수정 연계 (DB 외)**
> - `Order.create()` 초기 status `CREATED` (주문생성 Phase 코드 취합 — `stock.deducted` 수신 후 `PAYMENT_PENDING` 전이)
...
| 분류 | 대상 | 변경 내용 | 방법 | 비고 |
|------|------|-----------|------|------|
| outbox 생성 | 신규 테이블 | Commerce와 동일 구조 | @Entity 추가 → 자동 | 2026-04-22 실코드 대조 |
| processed_message 생성 | 신규 테이블 | Commerce와 동일 구조 (schema=`event`, `topic VARCHAR` 컬럼 포함) | @Entity 추가 → 자동 | |
| shedlock 생성 | 신규 테이블 | Outbox 스케줄러 분산 락 | 수동 CREATE TABLE | DDL 미실행 |
| event 엔티티 | `version` | BIGINT 컬럼 추가 (@Version) | 엔티티 필드 추가 → 자동 | |

> **합의 (2026-04-14):** OrderCreatedEvent / PaymentFailedEvent 모두 `List<OrderItem>(eventId, quantity)` 리스트 구조 채택 → Stock 신규 엔티티 추가 없음, 기존 event 테이블 quantity 컬럼 사용
...
-- ④ Commerce: order fail_reason 컬럼 제거 (엔티티에 fail_reason 필드 없음 — DB 측 적용 시 사용)
...
-- ⑪ Payment: refund_ticket — partial unique index (ticket_id WHERE status IN ('ACTIVE','COMPLETED'))
-- 동일 ticketId 활성 환불 요청 race condition 방어 (RefundServiceImpl.refundPgTicket).
-- FAILED 상태는 재시도 허용을 위해 UNIQUE 적용 대상에서 제외 — partial index 채택.
-- JPA @UniqueConstraint는 partial index를 표현할 수 없어 RefundTicketPartialIndexInitializer DDL 스크립트로 관리.
-- 코드 측 보강: existsActiveByTicketId 사전 체크 + DataIntegrityViolationException catch → REFUND_ALREADY_IN_PROGRESS
-- 적용 전 중복 활성 row 존재 여부 확인 필수:
--  SELECT ticket_id, COUNT(*) FROM payment.refund_ticket
--  WHERE status IN ('ACTIVE','COMPLETED') GROUP BY ticket_id HAVING COUNT(*) > 1;
...
| 분류 | 대상 | 변경 내용 | 방법 |
|------|------|-----------|------|
| saga_state 생성 | 신규 테이블 | refund_id(PK), order_id, payment_method, current_step, status, created_at, updated_at | @Entity (`payment/refund/domain/model/SagaState.java`) |
| refund 엔티티 | `version` | BIGINT 컬럼 (@Version, `Refund.java:30-32`) | 엔티티 필드 추가 → 자동 |
| refund_ticket 생성 | 신규 테이블 | refund_id, ticket_id, status(ACTIVE/COMPLETED/FAILED) — `payment/refund/domain/model/RefundTicket.java`. partial unique index `uk_refund_ticket_active` 는 ⑪ DDL 참조 | @Entity 추가 → 자동 |

### Commerce

| 분류 | 대상 | 변경 내용 | 방법 |
|------|------|-----------|------|
| ticket 엔티티 | `version` | BIGINT 컬럼 (@Version, `Ticket.java:62-65`) — refund.ticket.cancel Consumer 대응 | 엔티티 필드 추가 → 자동 |
