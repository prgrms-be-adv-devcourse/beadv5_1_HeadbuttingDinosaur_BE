...
|---|---|---|---|---|---|
| Payment | POST | `/api/payments/ready` ★ | `PaymentController#readyPayment` | 사용자 (commerce 주문 후) | (#4) 결제수단별(PG/WALLET/WALLET_PG) Payment 생성 |
| Payment | POST | `/api/payments/confirm` ★ | `PaymentController#confirm` | 사용자 / PG callback | (#4) PG 승인 + `payment.completed` Outbox 발행 |
| Payment | POST | `/api/payments/fail` ★ | `PaymentController#fail` | 사용자 / PG callback | (#4) PG 실패 + WALLET_PG 예치금 복구 + `payment.failed` Outbox 발행 |
...
| Wallet Internal | POST | `/internal/wallet/settlement-deposit` ★ | `WalletInternalController#depositFromSettlement` | settlement | (#7) 정산금 → 판매자 예치금 입금 |
