...
|---|---|---|---|---|
| GET | `/health` ★ | `GatewayHealthController#health` | k8s liveness/readiness probe, 모니터링 | (§2 MSA) gateway 라우팅 동작 확인용 헬스 체크 |
...
| 컴포넌트 | 위치 | 역할 |
|---|---|---|
| `JwtAuthenticationFilter` ★ | `apigateway/.../infrastructure/security/` | (§2 JWT) JWT 검증 (Authorization 헤더) → `X-User-Id` 헤더로 다운스트림 전달 |
| `RoleAuthorizationFilter` ★ | `apigateway/.../infrastructure/security/` | (§2 보안) path 별 권한(USER/SELLER/ADMIN) 검증 |
...
| `OAuthSuccessHandler` ★ | `apigateway/.../infrastructure/oauth/` | (§2 OAuth) Google OAuth 콜백 진입점 → member 모듈 위임 (`internalOAuthSignUpOrLogin`) |
| `OAuthFailureHandler` ★ | `apigateway/.../infrastructure/oauth/` | (§2 OAuth) OAuth flow 실패 처리 |
