...
주문 생성은 클라이언트 키가 아니라 **userId + cartId**를 기준으로 멱등 처리한다.

**어디서 막는가:**

| 레이어 | 역할 |
|--------|------|
| 서비스 레이어 | 활성 주문 존재 여부 판단 |
| DB 레이어 | 동시 요청 직렬화 (SELECT ... FOR UPDATE) |
| Gateway | 짧은 시간 반복 호출 차단 |

**무엇을 기준으로 중복 판단하는가:**
- 같은 userId + 같은 cartId + 아직 끝나지 않은 활성 주문
...
4. 활성 주문이 없을 때만 새 주문 생성
   → cart_version 일치 확인
   → Order 생성 (CREATED, expires_at = now + 10분)
...
차단 — 새 주문 생성 거부, 기존 주문 반환:
  CREATED, STOCK_RESERVED, PAYMENT_PENDING, PG_PENDING, PAID, REFUNDED
...
3. 승인 직전 상태 전이에 동시성 제어 적용
   → READY → PG_PENDING 전이에 @Version 낙관적 락
...
| API | 어디서 | 어떻게 |
|-----|--------|--------|
| **주문 생성** | 서비스 + DB | userId + cartId 기준 활성 주문 조회, SELECT FOR UPDATE 직렬화, 있으면 기존 주문 반환 |
...
### 폴링 vs SSE — 재고 선점 결과 전달 방식

주문 생성 후 Saga 흐름(재고 차감 → 결제)이 비동기로 진행되는 동안,
클라이언트에게 처리 결과를 어떻게 전달할지 확정 필요.

| 방식 | 장점 | 단점 |
|------|------|------|
| 폴링 (GET 반복 호출) | 구현 단순 | 불필요한 요청 발생, 실시간성 낮음 |
| SSE (Server-Sent Events) | 실시간 푸시, 불필요 요청 없음 | 구현 복잡, 연결 관리 필요 |

- 현재 권장: **폴링** (구현 단순)
- 최종 선택은 프론트-서버 팀 협의 후 확정
