...
### 4-6. Saga 결과 전달 — 폴링 (GET /api/orders/{orderId})

주문 생성 후 Saga 흐름(재고 차감 → 결제)은 비동기로 진행된다.
클라이언트는 `GET /api/orders/{orderId}`를 반복 호출하여 처리 결과를 확인한다. **폴링 방식으로 확정.**

**서버 측 구현 책임:**

`GET /api/orders/{orderId}` 응답에는 반드시 아래 필드가 포함되어야 한다.

| 필드 | 타입 | 설명 |
|------|------|------|
| `orderId` | UUID | 주문 ID |
| `status` | String (enum) | 현재 주문 상태 |
| `failReason` | String (nullable) | 실패 사유 — status가 FAILED / CANCELLED일 때만 값 있음 |
| `updatedAt` | Instant | 마지막 상태 변경 시각 |
...
- `GET /api/orders/{orderId}` 는 인증된 본인 주문만 조회 가능
...
| **Saga 결과 전달** | 클라이언트 → 서버 | **폴링** 방식으로 확정 — 주문 생성 후 클라이언트가 `GET /api/orders/{orderId}`를 반복 호출하여 상태 변화 감지 |
