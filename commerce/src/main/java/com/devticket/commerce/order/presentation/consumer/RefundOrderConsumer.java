...
public class RefundOrderConsumer {
    ...
    public void consumeOrderCancel(ConsumerRecord<String, String> record, Acknowledgment ack) {
        log.info("[refund.order.cancel] raw headers={}", record.headers());
        log.info("[refund.order.cancel] raw value={}", record.value());
        UUID messageId = extractMessageId(record.headers());
        String payload = PayloadExtractor.extract(objectMapper, record.value());
        try {
            refundOrderService.processOrderRefundCancel(messageId, record.topic(), payload);
            ack.acknowledge();
    ...
    public void consumeOrderCompensate(ConsumerRecord<String, String> record, Acknowledgment ack) {
        ...
        String payload = PayloadExtractor.extract(objectMapper, record.value());
        try {
            refundOrderService.processOrderCompensate(messageId, record.topic(), payload);
            ack.acknowledge();
        } catch (DataIntegrityViolationException e) {
            if (isProcessedMessageUniqueConflict(e)) {
    ...
    public void consumeRefundCompleted(ConsumerRecord<String, String> record, Acknowledgment ack) {
        ...
        String payload = PayloadExtractor.extract(objectMapper, record.value());
        try {
            refundOrderService.processRefundCompleted(messageId, record.topic(), payload);
            ack.acknowledge();
        } catch (DataIntegrityViolationException e) {
            if (isProcessedMessageUniqueConflict(e)) {
